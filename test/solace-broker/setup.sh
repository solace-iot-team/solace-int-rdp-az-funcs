...
# Run
  ...
  echo ">>> create broker service ..."
    playbook=$(assertFile $scriptName "$scriptDir/setup-broker.playbook.yml") || exit
    ansible-playbook \
      $playbook \
  ...
  echo ">>> configure broker service ..."
    playbook=$(assertFile $scriptName "$scriptDir/configure-broker.playbook.yml") || exit
